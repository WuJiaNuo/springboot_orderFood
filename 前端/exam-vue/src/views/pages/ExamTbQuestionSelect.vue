<template>
    <div>
        ...
        <TbQuestionSelectDialog
                ref="tbQuestionSelect"
...
<script>
    import TbQuestionSelectDialog from './ExamTbQuestionSelectDialog'
    ...
    export default {
        ...
        watch: {
            myvals: {
                handler (newVal) {
                    ...
                    if (newVal && newVal.length > 0) {
                        tbQuestionApi.getByIds(newVal).then(
                            response => {
            ...
            selectData: {
                handler (newVal) {
                    this.name = newVal.map(item => item.content).join(',')
                },
                immediate: false,
                deep: false
            }
        ...
        methods: {
            ...
            // 显示列表
            showUserSelect () {
                this.$refs.tbQuestionSelect.init()
            }
        }
    }
</script>
