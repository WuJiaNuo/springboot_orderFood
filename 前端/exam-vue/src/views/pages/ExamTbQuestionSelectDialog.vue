              ...
              <el-col :span="5" style="margin-left: 5px">
                <el-form-item prop="type">
                  <!--                  <el-input
                                      ...
                                      placeholder="4001:单选 4002:多选 4003:判断"
                                      @input="changeInput($event)"
                                      clearable>
                                    </el-input>-->
  ...
  methods: {
    ...
    init() {
      ...
      this.$nextTick(() => {
        ...
        this.$refs.tbQuestionTable.clearSelection()
        this.resetSearch();
        ...
        response => {
          ...
          if (this.selectData && this.selectData.length <= 10000) {
            ...
            this.$nextTick(function () {
              ...
              if (arr) {
                arr.forEach((row) => {
                  this.$refs.tbQuestionTable.toggleRowSelection(row, true)
                })
              }
            })
            //test end
    ...
    handleSelectionChange(selection, row) {
      if (row) {
            ...
            if (!selected) { //取消选中把默认值移除掉
              this.selectData.forEach((val, index) => {
                if (val.id == row.id) {
                  ...
                  console.log("has removed " + val.content)
                }
              })
        ...
        console.log('全选还是反选')
        let isCheckAll = this.$refs.tbQuestionTable.store.states.isAllSelected;
        //反选的时候 移除默认
        if (isCheckAll == false) {
          console.log('点击了反选')
          this.$nextTick(function () {
